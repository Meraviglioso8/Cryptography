...
int main(int argc, char* argv[])
{
    try
    {
        ...
        parameters.GenerateRandomWithKeySize(rng, 13312);
        RSA::PrivateKey privateKey(parameters);
        RSA::PublicKey publicKey(parameters);

        string filename1 = "/privateKey.txt";
        string filename2 = "/publicKey.txt";
        // Load private key from file
        
        try 
        {
            SaveKey(privateKey, filename1);
        }
        catch (Exception)
        {
            std::cout << "error loading private key\n";
        }
        ...
        try
        {
            SaveKey(publicKey, filename2);
        }
        catch (Exception)
        {
            std::cout << "error loading public key\n";
        }
        ...
        std::string myPlaintext;
        std::getline(std::cin, myPlaintext);
